"""
A collection of useful utility classes, methods, and items that are used for many different things.
"""
...
class EmbedUtil:
    """
    Used for creating and modifying embedded messages.
    """
    ...
    def __init__(self, bot, settings):
        ...
        self.embed_color = discord.Color.from_rgb(
            settings.embed['color']['r'],
            settings.embed['color']['g'],
            settings.embed['color']['b']
        )
        self.footer = settings.embed['footer']['text']
        self.footer_image = settings.embed['footer']['image']
        self.timestamp = bot.embed_ts
        self.show_author = settings.embed['show_author']
...
class Confirmation(menus.Menu):
    """
    Used for getting a yes or no confirmation from a user.
    """
    ...
    async def send_initial_message(self, ctx, channel):
        embed = ctx.bot.configs[ctx.guild.id].embed_util.get_embed(
            title=self.title,
            desc=self.msg,
            url=self.url
        )
...
class TimeLength:
    """
    Used in the conversion of a string input into a readable length of time.
    """
